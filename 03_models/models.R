...
png("03_models/QQplot_residuals.png", width = 800, height = 900)
layout(
...
for (i in 1:11) {
  ...
  qqnorm(m.stdres, 
         ...
         xlab="Normal Scores", 
         main=paste0(ind_names[i], "\n","Moran.I stat =",round(moran_i$estimate[1],2) ,"; p=", round(moran_p[i], 2)),         cex=1.2, cex.axis=1.3,cex.lab=1.3, cex.main=1.5)
  qqline(m.stdres)
}
...
#####################################################################
### Models with the same number of observation inside and outside reserves
#####################################################################
in_out <- mydata %>%
  group_by(protection,Site) %>% 
  summarise(n = n())

n <- unlist(in_out[10:18,"n"]) # sample size inside reserve for each site

sites <- unlist(in_out[10:18,"Site"])

## To select randomly samples inside and outside reserve for each site
sample_out <- mydata %>%
  filter(protection == "outside") %>%
  group_by(Site) %>% 
  nest() %>%  # creates a list-column of data frames          
  ungroup() %>% 
  arrange(Site) %>%
  mutate(n = n) %>% # add a column of targeted sample sizes
  mutate(samp = map2(data, n, sample_n)) %>% # sample n rows per site
  select(-data) %>% # remove the rest
  unnest(samp) %>%
  select(-"n") %>% # remove the "n" column
  relocate(Site, .after= Samples) %>% #move column Site back 
  bind_rows(filter(mydata, protection == "reserve")) # bind with the reserve samples

##### Re-run the previous models using this "sample_out" dataset
# mydata <- sample_out

# Same results but LFI becomes almost significant (pval = 0.057)
...
evidence <- c("Strong", "Strong", "No", "Moderate", "No",  "No",
...
## Save plot
png("03_models/Figure2_boxplot_predicted_revised.png", 
...
to_plot$Indicator[11] <- "Species Richness"
...
### Save the plot
ggsave("03_models/Figure3_protection_effects_revised.png",
       width=12, height=10,
       dpi = 300)
